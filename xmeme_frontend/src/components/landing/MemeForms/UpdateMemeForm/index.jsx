...
export const UpdateMemeForm = ({memeData, closeModal}) => {
  const { theme, setReload } = useContext(ThemeContext);

  return (


    <Wrapper>
      <h2>Update Meme Contents</h2>
      <Formik
        initialValues={{
          name: memeData.name,
        ...
        onSubmit={async ({ name, caption, url }, { setSubmitting, resetForm, setFieldValue }) => {
          try {
            const res = await axios({
              method: 'PATCH',
              url: `${process.env.BACKEND_URL}memes/${memeData.id}`,
              headers: {
                ...
                <Preview>
                  ...
                  <Item theme={theme}>
                    ...
                    <Card theme={theme}>
                      ...
                      <img alt={"Meme URL"} src={values.url === '' ? 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrRObJHq0Or1CgS4MLT-0tsrv__aGDKfw3zQ&usqp=CAU' : values.url} />
                      <Stats theme={theme}>
